...
- **Execution cards** keep the per-device narrative focused on min/avg/max latency plus **test run totals**, so spotting a noisy controller or a regression tied to a specific hardware build is immediate. Inline axis filters and “test run” terminology ensure the metrics match the report view.
