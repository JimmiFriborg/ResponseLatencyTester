...
> **Patch 3.8.1 (November 2025):** Ships as `latency-tester-v3.8.1.html` and addresses a JSX compile failure that prevented the UI from loading in browsers. The legacy `latency-tester-v3.8.html` now redirects to this patched build so existing bookmarks still work.
...
### Quick Start
1. Open `latency-tester-v3.8.1.html` in any modern browser (or use the legacy `latency-tester-v3.8.html` redirect)
...
- **Filename**: `latency-tester-v3.8.1.html`
...
┌─ Error #1 ─────────────────────┐
│ Error #1 (Duration: 4.133s)   │
└─────────────────────────────────┘
┌─ Error #2 ─────────────────────┐
│ Error #2 (Duration: 2.067s)   │
└─────────────────────────────────┘
```
...
## 🐛 **All v3.7 Issues Fixed**

| Issue | Status |
|-------|--------|
| Poor architecture | ✅ Complete rewrite |
| No multiple executions | ✅ Full support |
| Global requirements only | ✅ Per-execution |
| No comparison view | ✅ Implemented |
| No axis grouping | ✅ Default ON |
| Axis labels with distance | ✅ Clean notation |
| No include/exclude | ✅ Checkboxes |
| No edit/remove test case | ✅ Full CRUD |
| FPS not auto-detected | ✅ Auto-detection |
| No error grouping | ✅ By error number |
| Basic requirements | ✅ Min respond + Must not |
| No test report | ✅ Professional report |
...
**Testing**: ✅ Verified with AB6 CSV sample
