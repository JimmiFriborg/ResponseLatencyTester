...
- Establishes the React + TypeScript source of record in `app/src/`, bundled via Vite into a single-file `app/dist/latency-tester.html` portable artifact (no in-browser Babel dependency).
...
- Rehomes the active source, tests, and build outputs under the `app/` directory so legacy HTML builds and planning documents stay separate from the current codebase.
