...
- **React/TypeScript source of record** – The v4 portable artifact now lives under `app/`, which contains the React + TypeScript project and build tooling. Run `npm run build` from that folder to produce `app/dist/latency-tester.html`, a single self-contained file with no in-browser Babel dependency. The checked-in `app/latency-tester.html` serves as the Vite entrypoint/template for local development and bundling.
...
## Usage Guide
### Repository layout
- `app/` – current React/TypeScript source of record, build scripts, tests, and the generated `dist/latency-tester.html` bundle.
- `archive/` – legacy portable HTML builds and historical documentation kept for reference only.
- `release-candidates/` – v3.9 RC HTML builds plus their associated defect JSON files and helper scripts.
- Root docs (`v4.0-*`, `v3.*-release-notes`, guides) – planning artifacts and references that remain relevant across builds.
...
1. Change directories into `app/` and run `npm install` once to fetch the Vite + React toolchain.
2. Execute `npm run build` to generate `dist/latency-tester.html`, a single-file portable bundle with all CSS/JS inlined (no in-browser Babel required). The checked-in `latency-tester.html` inside `app/` is the Vite template used for this bundle; open the compiled file under `app/dist/` for the portable experience.
3. Open `app/dist/latency-tester.html` directly or serve it from any static host; hash-based routes (`#execution`, `#hardware`, etc.) keep each view addressable.
...
- Keep the compiled `app/dist/latency-tester.html` checked in so downstream users can open the portable artifact directly without invoking the build pipeline.
